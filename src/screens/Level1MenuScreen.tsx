...
export default function Level1MenuScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const deckProgress = useStore((s) => s.deckProgress);
  const masteryBest = useStore((s) => s.masteryChallengeBest);

  const masteredCount = (deckProgress.masteredHeadings || []).length;

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Level 1</Text>
      <Text style={styles.subtitle}>Vector Anchoring</Text>

      {/* Practice Mode */}
      <Pressable style={styles.modeCard} onPress={() => navigation.navigate('PracticeHome')}>
      ...
      <Pressable
        ...
        onPress={() => navigation.navigate('MasteryChallenge')}
      >
        <Text style={styles.modeCardTitle}>Mastery Challenge</Text>
        <Text style={styles.modeCardDesc}>
          All 36 headings, 1.2 seconds each
        </Text>
        {masteryBest && (
