...
export default function Level2ModeScreen() {
  ...
  const processAnswer = useCallback((userInput: string, timeMs: number) => {
    ...
    // Use the session manager to record the result
    // We pass a dummy wedgeId since we're using numpad - isCorrect is what matters
    const engine = engineRef.current;
    ...
    // Show feedback overlay for wrong answers
    if (!isCorrect) {
      setFeedback({
        state: 'red',
  ...
  // Get feedback border color for numpad area
  const numpadBorderColor = feedbackColor === 'green' ? '#00e676' : feedbackColor === 'amber' ? '#ffab00' : feedbackColor === 'red' ? '#ff5555' : 'transparent';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.numpadArea}>
        ...
        <View style={[styles.numpadWrapper, { borderColor: numpadBorderColor }]}>
          <Numpad
            onDigit={handleDigit}
            onClear={handleClear}
...
const styles = StyleSheet.create({
  ...
  numpadArea: { alignItems: 'center', marginTop: 8 },
  timerRow: { marginBottom: 12 },
  numpadWrapper: { borderWidth: 3, borderRadius: 16, padding: 8, borderColor: 'transparent' },
  controlRow: { flexDirection: 'row', justifyContent: 'center', gap: 12, marginTop: 24 },
